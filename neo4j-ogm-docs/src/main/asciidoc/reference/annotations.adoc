...
[WARNING]
You should write your `equals` and `hashcode` in a domain specific way for managed entities. *We strongly advise developers to not use the native graph id described by a `Long` field in combination with `@Id @GeneratedValue` in these methods*.
