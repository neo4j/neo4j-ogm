...
//??? Any specific link to place with hardware recommendation ?
...
For critical applications, these failures have to be anticipated, and workarounds built in.
Even if the driver handles some low level retries in case of instability, it is not always enough,
...
There is new API method `session.beginTransaction(Transaction.Type)` where Transaction.
Type is one of:
