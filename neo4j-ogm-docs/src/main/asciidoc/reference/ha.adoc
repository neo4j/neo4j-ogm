...
You can also provide additional instances in `URIS` property, separated by a comma.
`URI` still needs to be set and will be tried first, followed by entries from `URIS` property.
