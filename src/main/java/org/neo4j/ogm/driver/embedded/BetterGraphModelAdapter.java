 ...
 * @author vince
 */
public class BetterGraphModelAdapter extends ModelAdapter implements ResponseAdapter<Result, GraphModel> {

    /**
     * Reads the next row from the result object and transforms it into a GraphModel
     *
     * @param result
     * @return
     */
    public GraphModel adapt(Result result) {
        ...
        CLOSE_OBJECT(sb);
        try {
            return mapper.readValue(sb.toString().getBytes(), GraphModel.class);
        } catch (Exception e) {
