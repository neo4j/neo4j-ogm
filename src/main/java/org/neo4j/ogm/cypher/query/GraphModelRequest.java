 ...
 */
public class GraphModelRequest extends AbstractRequest {

    private final static String[] resultDataContents = new String[] {"graph"};

    public GraphModelRequest(Statement request) {
        this(request.getStatement(), request.getParameters());
    }

    public GraphModelRequest(String cypher) {
        this(cypher, Utils.map());
    }

    public GraphModelRequest(String cypher, Map<String, ?> parameters) {
