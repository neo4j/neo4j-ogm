...
The `Session` requires a URL with which to access the remote Neo4j database. For test environments, `Neo4jIntegrationTestRule.url()` can be used instead. TODO See the section on testing.
