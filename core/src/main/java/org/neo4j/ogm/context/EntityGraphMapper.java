...
public class EntityGraphMapper implements EntityMapper {
     ...
     * incremented by 1 one each time entering {@link #mapEntityReferences(Object, NodeBuilder, int, Compiler)}  and decremented
    ...
    private void link(Compiler cypherCompiler, DirectedRelationship directedRelationship, NodeBuilder nodeBuilder,
        ...
        if (relNodes.target != null) {
            ...
            if (isRelationshipEntity(relNodes.target)) {
                ...
                mapRelatedEntity(cypherCompiler, nodeBuilder, relationshipBuilder, currentDepth.get(), horizon, relNodes);
            }
        } else {
    ...
    private void mapRelatedEntity(Compiler compiler, NodeBuilder srcNodeBuilder,
        ...
        boolean alreadyVisitedNode = compiler.context().visited(relNodes.target, horizon);
        ...
        if (!alreadyVisitedNode || !selfReferentialUndirectedRelationship || relationshipFromExplicitlyMappedObject) {
            ...
            updateRelationship(compiler.context(), srcNodeBuilder, tgtNodeBuilder, relationshipBuilder, relNodes);
        }
    }
