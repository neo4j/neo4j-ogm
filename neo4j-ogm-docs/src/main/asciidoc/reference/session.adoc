...
* *schema load strategy* - uses metadata on domain entities and pattern comprehensions to retrieve nodes and relationships (default sinceNeo4j-OGM 3.0)

* *path load strategy* - uses paths from root node to fetch related nodes, `p=(n)-[0..]-()` (default beforeNeo4j-OGM 3.0)
