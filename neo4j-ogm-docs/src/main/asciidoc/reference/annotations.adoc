...
- sort relationships in @PostLoad method

You can sort either by a property of a related node or by relationship property.
To sort by relationship property you need to use relationship entity. See <<reference:annotating-entities:relationship-entity>>.
...
It is possible (but optional) to annotate a property of a node entity with @Id annotation, which signifies the property is a primary id.
A unique index is created for such property.
An entity with @Id annotation can be looked up by this primary id by using `Session.load(Class<T>, ID)` and `Session.loadAll(Class<T>, Collection<ID>)` methods.
...
[[reference:annotating-entities:rw-ordering]]
