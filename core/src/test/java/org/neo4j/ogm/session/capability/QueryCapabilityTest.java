 ...
 */
public class QueryCapabilityTest extends IntegrationTestRule {
	...
	public void shouldQueryForArbitraryDataUsingBespokeParameterisedCypherQuery() {
		...
		QueryResult results = session.query("MATCH (a:Actor) WHERE ID(a)={param} RETURN a.name as name",
	...
	public void modifyingQueryShouldReturnStatistics() {
		...
		QueryResult result = session.query("MATCH (a:Actor) SET a.age={age}", MapUtil.map("age", 5), false);
	...
	public void modifyingQueryShouldReturnResultsWithStatistics() {
		...
		session.save(new Actor("John"));
		session.save(new Actor("Colin"));
		QueryResult result = session.query("MATCH (a:Actor) SET a.age={age} RETURN a.name", MapUtil.map("age", 5), false);
		assertNotNull(result);
		assertNotNull(result.statistics());
		assertEquals(3, result.statistics().getPropertiesSet());
	...
	public void readOnlyQueryShouldNotReturnStatistics() {
		...
		session.save(new Actor("John"));
		session.save(new Actor("Colin"));
		QueryResult result = session.query("MATCH (a:Actor) RETURN a.name", Collections.EMPTY_MAP, true);
		assertNotNull(result);
