    ...
    private void resetRelation(Node node, FieldInfo field, Object currentValue) {
        ...
        if (INCOMING.equals(direction)) {
            rel = new MappedRelationship(identity, field.relationshipType(), node.getId(),
                currentValue.getClass(), DescriptorMappings.getType(field.typeParameterDescriptor()));
        } else if (OUTGOING.equals(direction)) {
            rel = new MappedRelationship(node.getId(), field.relationshipType(), identity,
                DescriptorMappings.getType(field.typeParameterDescriptor()), currentValue.getClass());
        }
            ...
            if (writer.forScalar()) {
                ...
                mappingContext.addRelationship(
                    new MappedRelationship(edge.getStartNode(), edge.getType(), edge.getEndNode(), edge.getId(),
                        source.getClass(), DescriptorMappings.getType(writer.typeParameterDescriptor())));
            } else {
                oneToMany.add(edge);
            }
        }
            ...
            if (relationshipEntity != null) {
                ...
                if (outgoingWriter != null) {
                    ...
                    relationshipsToRegister.add(
                        new MappedRelationship(edge.getStartNode(), edge.getType(), edge.getEndNode(), edge.getId(),
                            instance.getClass(), DescriptorMappings.getType(outgoingWriter.typeParameterDescriptor())));
                }
